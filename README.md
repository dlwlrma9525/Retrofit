...
REST는 2000년도 로이 필딩 (Roy Fielding)의 박사학위 논문에서 최초로 등장        
**Web(HTTP) 설계의 우수성과 장점을 최대한 활용할 수 있는 아키텍처** 로써 REST 발표.  

REST는 HTTP/1.1 Spec과 동시에 만들어졌는데, **HTTP Protocol을 정확히 의도에 맞게 활용한 디자인**     
기반으로 디자인 기준의 명확성, 의미적인 범용성을 지니므로 중간 계층 컴포넌트의 서비스 최적화 도움.          
...
그러므로 **서비스의 자유도 상승, API 구현의 단순화** 도움.          
...
HTTP Protocol 표준에서 사용하는 Last-Modified Tag or E-Tag로 캐싱 구현이 가능.  

**Self-descriptiveness (자체 표현 구조)**  
REST는 REST API Message 만 보고도 이해할 수 있는 자체 표현 구조로 구성.
...
**계층형 구조**  
REST Server는 다중 계층으로 구성될 수 있으며 보안, 로드 밸런싱, 암호화 계층 추가.       
구조상의 유연성을 추구하고 Proxy, Gateway 같은 Network 기반의 중간 매체를 사용할 수 있도록 한다.

--------------------------------------------------

## REST API 디자인 가이드

**첫 번째**, URI는 정보의 리소스(주소)만을 표현한다.  
**두 번째**, 리소스에 대한 행위는 HTTP Method(GET, POST, PUT, DELETE)로 표현한다.

--------------------------------------------------

## URI
**Uniform Resource Identifier, URI는 리소스를 식별하는데 사용되는 문자열**      
URI는 특정 Protocol을 사용하는 Network (일반적으로 World Wide Web) 기반의         
리소스의 주소 표현과 데이터 교환을 가능하게 한다.

**URI의 URL, URN**  
URN은 이름으로 비유할 수 있으며 URL은 주소로 비유할 수 있다.   
URN은 리소스을 식별하고 URL은 리소스를 찾기 위한 주소와 방법을 표현한다.

## URL
**Web 상에 수많은 정보가 (WebPage, Image, Video..) 위치하는 유니크한 주소 정보를 나타내는 URI**  
URI의 가장 일반적인 형태는 URL (Uniform Resource Locator, Web Address).    
URL은 단순히 Network를 통해 리소스를 가리키는 URI
...
**``//`` (Two Slash)**   
Authority가 없는 경우, Path는 두 개의 슬래시로 시작 불가.
...
**Port**   
Web Server Application의 할당된 Port 번호
...
**Fragment (Bookmark)**  
WebPage 문서 안에서 북마크를 통해서 특정한 지점으로 링크, 스크롤 기능    
...
GET /member/1
```
회원정보를 가져올 때 ``GET``, 회원 추가 시의 행위를 표현하고자 할 때는 ``POST`` Method를 사용하여 표현.
...
**Web Client가 Web Server로 요청의 목적/종류를 알리는 Method**
...
**``GET``를 통해 해당 리소스를 조회. 리소스를 조회하고 해당 Document에 대한 자세한 정보를 가져온다**  

> URL에 Query String이 보이는 방식

## ``POST``
**Web Client의 정보를 Web Server에게 전달하는 방식**  
**``POST``를 통해 해당 URI를 요청하면 리소스 생성**  
...
**REST API는 분명한 URI를 만들어 통신** 하므로 URI 경로의 마지막에는 슬래시 ``/``를 사용하지 않는다.
...
URI를 쉽게 읽고 해석하기 위해, 긴 URI Path 표현 시 하이픈을 사용해 가독성을 높일 수 있다.
...
## 자원을 표현하는 Colllection And Document
Collection과 Document로 URI 설계가 한 층 더 쉬워진다.   
**Document는 단순히 문서 또는 객체 하나.**     
**Collection은 문서들의 집합 또는 객체들의 집합.**     
...
직관적인 REST API를 위해서는 Collection과 Document 사용 시 단수, 복수로 더 쉬운 URI를 설계할 수 있다.  
...
만약 ``200``이나 ``4XX``관련 특정 코드 정도만 사용하고 있다면    
처리 상태에 대한 좀 더 명확한 상태코드 값 사용을 권장.

#### 상태코드
|Code      |Description                                                                   |
|----------|------------------------------------------------------------------------------|    
|``200``   |Client의 요청을 정상적으로 처리됨
...
|Code      |Description                                                                   |
|----------|------------------------------------------------------------------------------|      
|``400``   |Client의 요청이 부적절 할 경우의 응답 코드
|``401``   |Client가 인증되지 않은 상태에서 보호된 리소스를 요청했을 때의 응답 코드
|          |(로그인 하지 않은 유저가 로그인 했을 때, 요청 가능한 리소스를 요청했을 때)
|``403``   |User 인증상태와 관계 없이 응답하고 싶지 않은 리소스를 Client가 요청했을 때의 응답 코드
|          |(403 보다는 400이나 404를 사용할 것을 권고. 403 자체가 리소스가 존재한다는 뜻)
|``405``   |Client가 요청한 리소스에서는 사용 불가능한 Method를 이용했을 경우의 응답 코드
...
전세계에 있는 컴퓨터들이 Network(연결망)로 연결되어 데이터를 주고받을 수 있는 거대한 Network  
**Web이 동작하는 플랫폼**  
**컴퓨터 간 TCP/IP Protocol을 이용해 데이터를 주고받는 Network**
...
## IP Address
Internet 상의 여러 Host(컴퓨터) 간 패킷 교환 네트워크에서 **데이터를 주고받는 데 필요한 고유 주소**
...
전세계적으로 컴퓨터, 스마트폰, IoT..와 같은 디바이스에 모두 할당 x

## IPv6
**IPv4 주소의 고갈의 근본적인 해결책으로 도입된 IP 주소체계**  
2^128개의 IP주소 할당 가능  
IPv4가 적용된 디바이스에 당장은 적용 x
...
(Server 컴퓨터 사이에 공유기가 있으면, 공유기에 접속..)
...
사실, 컴퓨터는 Domain을 통해서 Server에 접속할 수 없고, 오직 IP주소로만 접속 가능!

## DNS (Domain Name System)
![](http://dl.dropbox.com/s/twtud63naicfgsp/DNS.png)
...
**Port가 필요한 이유는 컴퓨터 내에는 다양한 Application이 설치되어 있고**    
**각각의 프로그램들은 인터넷에 연결되어 있기 때문에.**     
**IP주소 만으로는 컴퓨터만 구분하므로 어떤 Application과 통신 해야하는지 구분해야 하기 때문이다.**
...
222.109.62.43 IP 주소를 가진 Web Server에 http Protocol로 접속  
(Web Browser는 기본적으로 http Protocol로 Web에 접근하므로 ``http://`` 생략하여 표현하기도 함)  
...
## Port Forwarding
**공인 IP가 부여된 공유기가 사설 IP를 할당받은 Web Server가 설치된 컴퓨터로 Toss!**  
공유기의 관리자 모드에서 설정 (해당 사설 IP의 컴퓨터의 해당 포트로 Client 접속을 Toss)  
...
* 절대 경로의 경우, 경로에 따라 URL의 Host 표현이 바뀔수 있다.    
  (접속은 IP주소로 하였으나 특정 WebPage에서 ``localhost``로 표현..)

## 상대경로
하나의 Web Server 안에 있는 리소스를 로드하거나, 다른 경로에 있는 링크로 이동 시 사용  
**현재 나의 위치에 따라 달라지는 경로**  

* ``/``는 부모 디렉토리 하위의 경로를 의미  
* ``http://localhost/path/1.html`` 상에서 ``../index.html``라는 경로는    
  path의 부모 디렉토리(여기서는 루트 디렉토리) 상의 ``index.html``를 의미  
